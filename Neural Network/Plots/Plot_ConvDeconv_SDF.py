...
project_folder = '/content/drive/My Drive/Colab Notebooks/ARCHER2_RUNS/results/Conv-Deconv/sdf/simple' # working folder path
os.chdir(project_folder) # changing the path

predictions1 = np.load('predictions3.npy')
Vx_test1 = np.load('Vx_test3.npy')
Vy_test1 = np.load('Vy_test3.npy')
Rho_test1 = np.load('Rho_test3.npy')
...
history_data = np.load('training_history_data3.npz')
...
project_folder = '/content/drive/My Drive/Colab Notebooks/ARCHER2_RUNS/results/Conv-Deconv/sdf/both' # working folder path
os.chdir(project_folder) # changing the path
...
project_folder = '/content/drive/My Drive/Colab Notebooks/ARCHER2_RUNS/results/Conv-Deconv/sdf/output' # working folder path
os.chdir(project_folder) # changing the path
predictions2 = np.load('predictions4.npy')
test_scores = np.load('test_scores4.npy')
...
history_data = np.load('training_history_data4.npz')
...
for i in range(predictions1.shape[1]):
  vel = np.sqrt(predictions1[1][i,:,:,0]**2 + predictions1[0][i,:,:,0]**2)
  vel_or = np.sqrt(Vx_test1[i,:,:,0]**2 + Vy_test1[i,:,:,0]**2)

  vals = np.abs((vel- vel_or)/vel_or)
  filtered_vals = vals[np.isfinite(vals)]
  rel_err.append(np.mean(filtered_vals)*100)

  val2 = predictions1[2][i,:,:,0]
  val2_or = Rho_test1[i,:,:,0]
  vals2 = np.abs((val2- val2_or)/val2_or)
  filtered_vals2 = vals2[np.isfinite(vals2)]
  rel_rho.append(np.mean(filtered_vals2)*100)
...
fig, axs = plt.subplots(1, 4, figsize=(25, 6))  # 1 row, 5 columns
...
im5 = axs[3].imshow(predicted_velocity_magnitude10000, cmap='viridis',origin='lower')
axs[3].set_title('Between and before output')
axs[3].axis('off')
...
im4 = axs[3].imshow(predicted_rho10000, cmap='viridis',origin='lower')
axs[3].set_title('Between and before output')
axs[3].axis('off')
