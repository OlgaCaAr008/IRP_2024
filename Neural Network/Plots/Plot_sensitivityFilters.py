...
project_folder = '/content/drive/My Drive/Colab Notebooks/ARCHER2_RUNS/results/filters/4layers' # working folder path
os.chdir(project_folder) # changing the path

# Load the feature maps
data = np.load('feature_maps_all300.npz')
...
for i, key in enumerate(sorted(data.files)):  # Sort keys to maintain layer order
    ...
    num_rows = (num_feature_maps + 24) // 25

    # Create a figure for this layer
    fig, axes = plt.subplots(num_rows, 25, figsize=(20, num_rows*2), squeeze=False)
    fig.suptitle(f'Layer {i+1} Feature Maps', fontsize=20)

    for j in range(num_feature_maps):
        row, col = divmod(j, 25)  # Determine the row and column to place the subplot
        ax = axes[row, col]
    ...
    for k in range(j + 1, num_rows * 25):
        row, col = divmod(k, 25)
...
test_loss55 = np.load('test_scores_150.npy')
print(test_loss55[0])
test_loss55 = np.load('test_scores_300.npy')
print(test_loss55[0])
test_loss55 = np.load('test_scores_600.npy')
print(test_loss55[0])
