...
factor = 0.25
lim = int(geometry.shape[0]*factor) 

geometry = geometry[0:lim,:,:]
vx = field[0:lim,:,:,1] # n x Ny x Nx
vy = field[0:lim,:,:,2] # n x Ny x Nx
rho = field[0:lim,:,:,0]
...
np.save('in_geo.npy',Input_test1)

# Save predictions
np.save('test_scores_25.npy',test_loss)

np.save('predictions_25.npy', predictions)
np.save('Vx_test_25.npy', Vx_test)
np.save('Vy_test_25.npy', Vy_test)
np.save('Rho_test_25.npy', Rho_test)
