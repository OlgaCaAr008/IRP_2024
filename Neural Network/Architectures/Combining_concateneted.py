...
concat_x = layers.Concatenate()([output1[0], output2[0]])
concat_y = layers.Concatenate()([output1[1], output2[1]])
concat_rho = layers.Concatenate()([output1[2], output2[2]])
combined_output_x = layers.Dense(1, activation='linear')(concat_x)
combined_output_y = layers.Dense(1, activation='linear')(concat_y)
combined_output_rho = layers.Dense(1, activation='linear')(concat_rho)

# Create the fusion model
fusion_model = models.Model(inputs=[model1.input, model2.input], outputs=[combined_output_x, combined_output_y, combined_output_rho])
...
# Save predictions
np.save('test_scores.npy',test_loss)

np.save('predictions.npy', predictions)
np.save('Vx_test.npy', Vx_test)
np.save('Vy_test.npy', Vy_test)
np.save('Rho_test.npy', Rho_test)
